...
import {
  ...
  deleteVariableById,
  getVariableById,
  getVariableByNameAndScope,
  updateWorkspaceNameFields,
  updateWorkspaceParameterTypes,
  updateWorkspaceStateTypes,
} from "../generator/main";
import { LabelGroup } from "../generator/solidity";

Blockly.Extensions.register("declare_typed_variable", function () {
  var block = this;

  if (!this.getVariableNameField) {
    throw new Error("missing getVariableNameField method");
  }

  if (!this.getVariableType) {
    throw new Error("missing getVariableType method");
  }

  if (!this.getVariableGroup) {
    throw new Error("missing getVariableGroup method");
  }

  if (!this.getVariableScope) {
    throw new Error("missing getVariableScope method");
  }
  ...
  this.declareOrUpdateVariable = function (name, force = false) {
    ...
    if (!getVariableByNameAndScope(name, scope, group)) {
      newName = name;
    } else {
      var count = 2;
      var newName = name + count;
      while (getVariableByNameAndScope(newName, scope, group)) {
        count++;
        newName = name + count;
      }
    }

    var variable = getVariableById(this.workspace, this.id);
    if (!variable) {
      createVariable(this.workspace, group, type, newName, scope, this.id);
    } else {
      variable.name = newName;
    }

    if (force) {
      console.log("this.getVariableNameField()", this.getVariableNameField());
      this.getVariableNameField().setValue(newName);
    }

    updateWorkspaceNameFields(this.workspace);
  ...
  this.setOnChange(function (event) {
    updateWorkspaceNameFields(this.workspace);
    updateWorkspaceStateTypes(this.workspace);
    updateWorkspaceParameterTypes(this.workspace);
    ...
    if (event.type === "move" && !!event.oldParentId) {
      if (!!getVariableById(this.workspace, this.id)) {
        deleteVariableById(this.workspace, this.id);
      }
    }
...
Blockly.Blocks["contract_state"] = {
  init: function () {
    ...
    this.getVariableGroup = function () {
      return LabelGroup.LABEL_GROUP_STATE;
    };
...
Blockly.Blocks["contract_state_get"] = {
  init: function () {
    this.appendDummyInput().appendField(
      new Blockly.FieldDropdown([
        ["select state...", LabelGroup.UNDEFINED_NAME],
      ]),
      "STATE_NAME"
    );
...
Blockly.Blocks["contract_state_set"] = {
  ...
  validate: function (stateNameVariableId) {
    ...
    setTimeout(function () {
      updateWorkspaceStateTypes(workspace);
    }, 1);
...
Blockly.Blocks["contract_method_parameter"] = {
  init: function () {
    ...
    this.getVariableGroup = function () {
      return LabelGroup.LABEL_GROUP_PARAMETER;
    };
    this.getVariableScope = function () {
      var scope = this.getParent();
      while (
...
Blockly.Blocks["contract_method_parameter_get"] = {
  init: function () {
    this.appendDummyInput().appendField(
      new Blockly.FieldDropdown([
        ["select param...", LabelGroup.UNDEFINED_NAME],
      ]),
      "PARAM_NAME"
    );
    this.setOutput(true, null);
...
Blockly.Blocks["contract_method"] = {
  init: function () {
    ...
    this.getVariableGroup = function () {
      return LabelGroup.LABEL_GROUP_METHOD;
    };
    this.getVariableScope = function () {
      var scope = this.getParent();
      while (!!scope && scope.type !== "contract") {
...
Blockly.Blocks["contract_method_call"] = {
  init: function () {
    this.appendDummyInput()
      ...
      .appendField(
        new Blockly.FieldDropdown([
          ["select method...", LabelGroup.UNDEFINED_NAME],
        ]),
        "METHOD_NAME"
      );
    this.setPreviousStatement(true, null);
...
Blockly.Blocks["controls_for"] = {
  init: function () {
    ...
    this.getVariableGroup = function () {
      return LabelGroup.LABEL_GROUP_VARIABLE;
    };
    this.getVariableScope = function () {
      return this;
    };
